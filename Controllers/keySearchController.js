...
module.exports = {
    ...
    count_search: (req,res) => {
        ...
        async.parallel([
            (cb) => {
                KeySearch.find().sort({"CountSearch": -1}).limit(limit).skip(skip)
            ...
            success(res,"Lấy từ khóa tìm kiếm thành công", {KeySearch: results[0], count: results[1]})
        })
    },
    ...
    count_search_user: (req,res) => {
        ...
        async.parallel([
            (cb) => {
                KeySearch.find({"ListIdUser.IdUser": req.user._id}).limit(limit).skip(skip)
                .sort({"ListIdUser.Counts": -1})
                .select("Key CountSearch ListIdUser")
                .exec((e,k)=>e?cb(e):cb(null,k))
            },
            (cb) => {
                KeySearch.count({"ListIdUser.IdUser": req.user._id})
                .exec((e,c)=>e?cb(e):cb(null,c))
            }
        ], (e,results) => {
            if(e) return error_500(res,e);
            success(res,"Lấy từ khóa tìm kiếm của user thành công", {KeySearch: results[0], count: results[1]})
        })
    }
}